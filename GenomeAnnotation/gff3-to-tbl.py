...
# Edited for python3 by SLAV, 2019/02/13
...
def print_mrna(f, incomplete, locus_tag, product):
	...
	# ---- SLAV -----
	# print("\t\t\tprotein_id\tgnl|%s|%s" % (args.centre, locus_tag)) # original
	if args.centre != '':
...
def print_gene(f, gene_product_table, translations):
	...
	if "gene" in name:
		...
		gene = name # SLAV
		# gene = re.sub("[-_][0-9]+$", "", re.sub(r"\|.*$", "", name)) # original

	# ---- SLAV -----
	# locus_tag = "%s_%s" % (args.locustag, f.qualifiers["ID"][0]) # original
	...
	product = "hypothetical protein" if gene.startswith("orf") else \
		gene_product_table[gene] # SLAV
		# gene_product_table[re.sub("[-_].*$", "", gene)] # original
	# ---- SLAV -----
	...
	if 'pseudo' in f.qualifiers:
		print("\t\t\tpseudo")
	if 'pseudogene' in f.qualifiers: #SLAV
		print("\t\t\tpseudogene\t%s" % f.qualifiers['pseudogene'][0]) #SLAV
...
# parser.add_argument("--centre", help="Sequencing centre ID") # original
parser.add_argument("--centre", help="Sequencing centre ID", default='') # SLAV
# parser.add_argument("--locustag", help="Locus tag prefix") # original
parser.add_argument("--locustag", help="Locus tag prefix", default='') # SLAV
...
""" 
SLAV notes:
gene product file looks like: 

gene	product
APN2	abasic endonuclease/DNA lyase
lorf	LORF
MAT1-2-1	MAT1-2-1
MAT1-2-L	MAT1-2-L
SLA2	Src Like Adaptor 2

While the protein sequences have the name of the mRNA because the cds typically depend from it rather than from the gene
